...
public partial class MainForm : Form
{
    ...
    public MainForm(SoapClient soap, string user, bool admin, string cliente)
    {
        _soap = soap; _user = user; _admin = admin; _cliente = cliente;
        InitializeComponent();
    }
    ...
    private void DoDepositar(TabPage t)
    {
        ...
        btnDepositar.Click += async (s, e) => { try { var r = await Task.Run(() => _soap.Depositar(txtCuenta.Text.Trim(), txtMonto.Text.Trim(), txtMoneda.Text.Trim())); lblResultado.Text = r.Exitoso ? $"OK: {r.Mensaje} - Saldo: {r.Saldo:F2}" : $"ERROR: {r.Mensaje}"; lblResultado.ForeColor = r.Exitoso ? Color.Green : Color.Red; } catch (Exception ex) { lblResultado.Text = $"Error: {ex.Message}"; lblResultado.ForeColor = Color.Red; } };
    ...
    private void DoRetirar(TabPage t)
    {
        ...
        var lblResultado = new Label { Location = new(20, 160), AutoSize = true };
        btnRetirar.Click += async (s, e) => { try { var r = await Task.Run(() => _soap.Retirar(txtCuenta.Text.Trim(), txtMonto.Text.Trim(), txtMoneda.Text.Trim())); lblResultado.Text = r.Exitoso ? $"OK: {r.Mensaje} - Saldo: {r.Saldo:F2}" : $"ERROR: {r.Mensaje}"; lblResultado.ForeColor = r.Exitoso ? Color.Green : Color.Red; } catch (Exception ex) { lblResultado.Text = $"Error: {ex.Message}"; lblResultado.ForeColor = Color.Red; } };
    ...
    private void DoSaldo(TabPage t)
    {
        ...
        btnConsultar.Click += async (s, e) => { try { var r = await Task.Run(() => _soap.ConsultarSaldo(txtCuenta.Text.Trim())); lblResultado.Text = r.Exitoso ? $"Saldo: {r.Saldo:F2}" : $"ERROR: {r.Mensaje}"; lblResultado.ForeColor = r.Exitoso ? Color.Green : Color.Red; } catch (Exception ex) { lblResultado.Text = $"Error: {ex.Message}"; lblResultado.ForeColor = Color.Red; } };
    ...
    private void DoCuentas(TabPage t)
    {
        ...
        btnListar.Click += async (s, e) => { try { var list = await Task.Run(() => _soap.ListarCuentasPorCliente(txtCliente.Text.Trim())); dgv.DataSource = new System.ComponentModel.BindingList<CuentaResumen>(list); } catch (Exception ex) { MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); } };
    ...
    private void DoMisCuentas(TabPage t)
    {
        ...
        var dgv = new DataGridView { Location = new(10, 50), Size = new(850, 400), ReadOnly = true, AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill };
        btnListar.Click += async (s, e) => { try { var list = await Task.Run(() => _soap.ListarCuentasPorCliente(_cliente)); dgv.DataSource = new System.ComponentModel.BindingList<CuentaResumen>(list); } catch (Exception ex) { MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); } };
    ...
    private void DoClientes(TabPage t)
    {
        ...
        var dgv = new DataGridView { Location = new(10, 50), Size = new(850, 400), ReadOnly = true, AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill };
        btnListar.Click += async (s, e) => { try { var list = await Task.Run(() => _soap.ListarClientes()); dgv.DataSource = new System.ComponentModel.BindingList<ClienteResumen>(list); } catch (Exception ex) { MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); } };
        t.Controls.AddRange(new Control[] { btnListar, dgv });
    }
    ...
    private void DoElimCuenta(TabPage t)
    {
        ...
        btnEliminar.Click += async (s, e) => { try { var r = await Task.Run(() => _soap.EliminarCuenta(txtCuenta.Text.Trim())); lblResultado.Text = r.Exitoso ? $"OK: {r.Mensaje}" : $"ERROR: {r.Mensaje}"; lblResultado.ForeColor = r.Exitoso ? Color.Green : Color.Red; } catch (Exception ex) { lblResultado.Text = $"Error: {ex.Message}"; lblResultado.ForeColor = Color.Red; } };
    ...
    private void DoMovimientos(TabPage t)
    {
        ...
        var dgv = new DataGridView { Location = new(10, 50), Size = new(850, 400), ReadOnly = true, AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill };
        btnListar.Click += async (s, e) => { try { var list = await Task.Run(() => _soap.ListarMovimientos(txtCuenta.Text.Trim())); dgv.DataSource = new System.ComponentModel.BindingList<MovimientoModel>(list); } catch (Exception ex) { MessageBox.Show($"Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); } };
