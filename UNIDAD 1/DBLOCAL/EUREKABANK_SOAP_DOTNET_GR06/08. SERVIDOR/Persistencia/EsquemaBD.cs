...
END";
        ...
        private static void AsegurarReferencia(SqlConnection cn, string tabla, string campoCodigo, params string[] valores)
        {
            ...
            var columnas = tabla.EndsWith("moneda") ? "chr_monecodigo, vch_monedescripcion" :
                           tabla.EndsWith("sucursal") ? "chr_sucucodigo, vch_sucunombre, vch_sucuciudad, vch_sucudireccion, int_sucucontcuenta" :
                           "chr_emplcodigo, vch_emplpaterno, vch_emplmaterno, vch_emplnombre, vch_emplciudad, vch_empldireccion";

            using var check = cn.CreateCommand();
            check.CommandText = $"SELECT COUNT(1) FROM {tabla} WHERE {campoCodigo} = @codigo";
            check.Parameters.AddWithValue("@codigo", codigo);
