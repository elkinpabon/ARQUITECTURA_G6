...
public class TicketPremiumApiService(HttpClient httpClient) : ITicketPremiumApiService
{
    ...
    {
        var response = await httpClient.GetAsync("api/partidos/disponibles");
    ...
    {
        var response = await httpClient.GetAsync($"api/partidos/{codigoPartido}/localidades");
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await httpClient.PostAsJsonAsync("api/ventas", request, JsonOptions);
    ...
    {
        var response = await httpClient.GetAsync($"api/reportes/ventas/{codigoPartido}");
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await httpClient.PostAsJsonAsync("api/auth/login", request, JsonOptions);
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException(await ObtenerMensajeErrorAsync(response));
        }
