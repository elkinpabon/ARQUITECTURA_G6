...
/** Controlador MVC del cliente web (TicketPremium FIFA 2026). */
public class TicketController {
    ...
    /** Simula la amortizacion de un credito (sin comprar) para un subtotal de carrito. */
    public Resultado simularCredito(BigDecimal subtotal, BigDecimal entrada,
                                    int numCuotas, BigDecimal tasaMensual) {
        if (!sesion.activa()) return error("Sesion no iniciada.");
        return ws().simularCredito(subtotal, entrada, numCuotas, tasaMensual);
    }
    ...
    public List<Factura> misFacturas() {
        if (!sesion.activa()) return Collections.emptyList();
        return sesion.isAdmin()
                ? ws().listarTodasFacturas(sesion.getIdUsuario())
                : ws().misFacturas(sesion.getIdUsuario());
    }
