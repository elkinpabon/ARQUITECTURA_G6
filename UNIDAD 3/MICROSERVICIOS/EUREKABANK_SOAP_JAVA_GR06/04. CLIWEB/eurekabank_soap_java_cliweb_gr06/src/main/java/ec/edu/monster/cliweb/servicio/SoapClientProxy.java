...
final class SoapClientProxy implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        try {
            ...
            Object wrapper = requestType.getConstructor().newInstance();
            populate(method, wrapper, args == null ? new Object[0] : args);
            String payload = marshal(requestType, wrapper, requestInfo.localName());
            HttpRequest request = HttpRequest.newBuilder(endpoint).timeout(REQUEST_TIMEOUT)
                    .header("Content-Type", "text/xml; charset=UTF-8")
                    .header("SOAPAction", "\"\"")
                    .POST(HttpRequest.BodyPublishers.ofString(payload, StandardCharsets.UTF_8)).build();
            return readResponse(HTTP.send(request,
                    HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8)), responseType);
            ...
            throw new RuntimeException("Error invocando SOAP en " + endpoint + ": " + e.getMessage(), e);
        }
    }
    ...
    private static <A> A required(A annotation, String name, Method method) {
        if (annotation == null) throw new IllegalStateException(name + " ausente en " + method.getName());
        return annotation;
    }

    private static void populate(Method serviceMethod, Object wrapper, Object[] args) throws Exception {
        Annotation[][] annotations = serviceMethod.getParameterAnnotations();
        for (int i = 0; i < annotations.length; i++) {
            WebParam webParam = null;
            for (Annotation annotation : annotations[i])
                if (annotation instanceof WebParam value) { webParam = value; break; }
            if (webParam == null) throw new IllegalStateException("@WebParam ausente en parametro " + i);
            ...
            for (Method candidate : wrapper.getClass().getMethods())
                if (candidate.getName().equals(setterName) && candidate.getParameterCount() == 1) {
                    setter = candidate; break;
                }
            if (setter == null) throw new IllegalStateException("No existe " + setterName + " en "
    ...
    private static String marshal(Class<?> type, Object wrapper, String operation) throws Exception {
        Marshaller marshaller = JAXBContext.newInstance(type).createMarshaller();
        marshaller.setProperty(Marshaller.JAXB_FRAGMENT, true);
        StringWriter content = new StringWriter();
        marshaller.marshal(new JAXBElement(new QName(NS, operation), type, wrapper), content);
    ...
    private static Object readResponse(HttpResponse<String> response, Class<?> type) throws Exception {
        Document document;
        try { document = parse(response.body()); }
        catch (Exception e) {
        ...
        if (bodies.getLength() == 0) throw new RuntimeException("Respuesta sin SOAP Body");
        Element content = firstElement(bodies.item(0));
        if (content == null) throw new RuntimeException("SOAP Body vacio");
        if ("Fault".equals(content.getLocalName())) {
            NodeList messages = content.getElementsByTagName("faultstring");
            String message = messages.getLength() == 0 ? content.getTextContent().trim()
                    : messages.item(0).getTextContent().trim();
            throw new RuntimeException("SOAP Fault: " + message);
        }
        if (response.statusCode() < 200 || response.statusCode() >= 300)
            throw new RuntimeException("HTTP " + response.statusCode() + " desde el endpoint SOAP");
        Object wrapper = JAXBContext.newInstance(type).createUnmarshaller()
                .unmarshal(new DOMSource(content), type).getValue();
        try { return type.getMethod("getReturn").invoke(wrapper); }
        catch (NoSuchMethodException e) { return type.getMethod("isReturn").invoke(wrapper); }
    }
